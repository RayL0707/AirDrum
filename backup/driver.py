...
class AirDrum(object):
	def __init__(self):
		...
		pass
	def gesound(self):
		drum1 = AudioSegment.from_wav("audio/kick.wav")
		drum2 = AudioSegment.from_wav("audio/snare.wav")
		drum3 = AudioSegment.from_wav("audio/cymbals.wav")
		drum4 = AudioSegment.from_wav("audio/hat.wav")
		return drum1,drum2,drum3, drum4
	def plays(self,drum, t, lens,trigger, db = 0):
		if trigger:
			#time.sleep(t)
			if lens != -1:
				sound = drum[:lens] + db
	...
	def main(self):
		...
		while True:
			...
			#print a,s,k,l
			t1=Thread(target = self.plays, args=(drum1,0.01,600,a,+25,))
			t2 = Thread(target = self.plays, args=(drum2,0.01,400,s,))
			t3=Thread(target = self.plays, args=(drum3, 0.03, 500,k, -5,))
			t4 = Thread(target = self.plays, args=(drum4,0.01,400,l, -5,))
			t1.setDaemon(True)
			t2.setDaemon(True)
			t3.setDaemon(True)
			t4.setDaemon(True)
...
if __name__ == "__main__":
	try:
		...
		d.getoutput()
		d.controller.remove_listener(listener)
	exit
